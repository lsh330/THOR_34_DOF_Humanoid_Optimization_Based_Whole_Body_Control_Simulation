...
```math
[p]_{\times} [p]_{\times}^T = \|p\|^2 I_{3} - p p^T
```

 (음의 준정치)

이로부터 공간 관성 식 (2.14)에서

```math
I_{cm} + m[c]_{\times} [c]_{\times}^T
```
...
따라서 $\dot{R} R^T$ 는 반대칭이므로 어떤 벡터 $\omega$ 에 대해
...
식 (1.7)의 $R$ 을 $\mathbf{q} = [w, x, y, z]^T$ 에 대해 편미분하고,
...
RNEA의 역전파에서 `X_up[i].T @ f[i]` 는 정확히

```math
X_{up,i}^T = X_{up,i}^{-T}
```

 (힘 변환의 역)를 사용함을 주목하라 — 즉 `X_up[i]`가 운동 변환이면 그 전치가 힘을 "역방향"으로 변환한다.
...
```math
m_{pelvis}/m_{finger} \approx 10.6/0.3 \approx 35
```
