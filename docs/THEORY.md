...
구체적 계산: 몸체 $i$ 의 CoM 위치 $c_{i}$ 와 전체 CoM $c$ 의 차이
...
```
 ...
 N·s/m → $d_{n} = 300$ 은 과감쇠(Overdamped) 영역
...
)와 달리, 반암시적 오일러는 설정 업데이트에 **새 속도** $v_{k+1}$ 을 사용한다.
...
**성질 3**: $[p]_{\times}^2 = p p^T - \lVert p\rVert^2 I_{3}$

**증명**: $( p \times q) \times r = (q r^T - r q^T)p$ 에서 행렬 형태로 변환하면 확인 가능. $\quad \checkmark$

**성질 4**: $[p]_{\times} [p]_{\times}^T = \lVert p\rVert^2 I_{3} - p p^T$ (음의 준정치)
