...
- **부록**
  - [A. 기호 일람 (Notation Summary)](#부록-a-기호-일람-notation-summary)
  - [B–G. 부록 (별도 문서)](THEORY_APPENDIX.md)
...
```math
SE(3) = \left\{ (p, R) \;\middle|\; p \in \mathbb{R}^3,\; R \in SO(3) \right\}
```
...
단위 쿼터니언(Unit Quaternion)은 $\lVert \mathbf{q}\rVert = 1$ 을 만족해야 한다.
...
```math
\mathbf{q} \otimes [0, \mathbf{v}]^T = \begin{bmatrix} -x v_x - y v_y - z v_z \\ w v_x + y v_z - z v_y \\ w v_y + z v_x - x v_z \\ w v_z + x v_y - y v_x \end{bmatrix}
```
...
```math
v'_x = (1 - 2y^2 - 2z^2)v_x + 2(xy - wz)v_y + 2(xz + wy)v_z
```

**(1.6b)**

```math
v'_y = 2(xy + wz)v_x + (1 - 2x^2 - 2z^2)v_y + 2(yz - wx)v_z
```

**(1.6c)**

```math
v'_z = 2(xz - wy)v_x + 2(yz + wx)v_y + (1 - 2x^2 - 2y^2)v_z
```
...
**검증**: $\mathbf{q} = [1, 0, 0, 0]^T$ (항등 쿼터니언) → $R = I\_{3}$. $\quad \checkmark$
...
```math
q = \underbrace{[p_x, p_y, p_z]}_{\text{기저 위치(3)}}, \underbrace{[w, x, y, z]}_{\text{쿼터니언(4)}}, \underbrace{[q_1, q_2, \ldots, q_{34}]}_{\text{관절 각도(34)}}
```
...
```math
v = \underbrace{[\omega_x, \omega_y, \omega_z]}_{\text{기저 각속도(3)}}, \underbrace{[\dot{p}_x, \dot{p}_y, \dot{p}_z]}_{\text{기저 선속도(3)}}, \underbrace{[\dot{q}_1, \dot{q}_2, \ldots, \dot{q}_{34}]}_{\text{관절 속도(34)}}
```
...
**리 대수 $\mathfrak{se}(3)$**: 무한소 변환의 공간 — 정확히 공간 속도 $[\omega; v\_{lin}]$ 의 공간이다.
...
```math
\exp: \mathfrak{se}(3) \to SE(3), \quad \exp\left(\begin{bmatrix} [\omega]_{\times} & v \\ 0 & 0 \end{bmatrix} t\right) = \begin{bmatrix} R(t) & p(t) \\ 0 & 1 \end{bmatrix}
```
...
쿼터니언 표현은 $SO(3) \cong S^3 / \mathbb{Z}\_{2}$ (3차원 구면의 이중 피복)를 통해 회전을 파라미터화하며, 짐벌 락(Gimbal Lock)이 없고 계산이 효율적이다.

**쿼터니언과 회전 행렬의 저장 효율**:
- 회전 행렬: $3 \times 3 = 9$ 실수, 6개 제약 ($R^TR = I$, $\det R = 1$)
- 오일러 각: 3 실수, 짐벌 락 발생 가능
- 쿼터니언: 4 실수, 1개 제약 ($\lVert \mathbf{q}\rVert = 1$), 짐벌 락 없음
...
```math
m_{THOR} = m_{THORMANG3} \times \frac{65}{42} \approx m_{THORMANG3} \times 1.548
```

**(1.12)**

```math
l_{THOR} = l_{THORMANG3} \times \frac{1.78}{1.375} \approx l_{THORMANG3} \times 1.295
```
...
```math
I_{THOR} = I_{THORMANG3} \times \frac{65}{42} \times \left(\frac{1.78}{1.375}\right)^2 \approx I_{THORMANG3} \times 2.597
```
...
강체의 운동(Twist, 비틀림)은 각속도 $\omega \in \mathbb{R}^3$ 와 선속도 $v\_{lin} \in \mathbb{R}^3$ 로 구성된 6차원 벡터다.

**(2.1)**

```math
\mathbf{v} = \begin{bmatrix} \omega \\ v_{lin} \end{bmatrix} \in \mathbb{R}^6
```

> **규약**: Featherstone(2008) 및 이 프로젝트는 **omega-first** 규약을 사용한다. 상위 3개 성분이 각속도, 하위 3개 성분이 선속도다.

- 각속도 벡터 $\omega$: 공간 내 회전 속도 $[\text{rad/s}]$
- 선속도 벡터 $v\_{lin}$: 기준점의 이동 속도 $[\text{m/s}]$
...
```math
\mathbf{f} = \begin{bmatrix} \tau \\ f \end{bmatrix} \in \mathbb{R}^6
```
...
```math
P = \mathbf{f}'^T \mathbf{v}' = (X^{-T} \mathbf{f})^T (X \mathbf{v}) = \mathbf{f}^T X^{-1} X \mathbf{v} = \mathbf{f}^T \mathbf{v}
```
...
```math
v'_{lin} = v_{lin} - p \times \omega = v_{lin} + \omega \times p
```

여기서

```math
p \times \omega = [p]_{\times} \omega
```
...
는 반대칭 행렬(Skew-Symmetric Matrix):
...
```math
\begin{bmatrix} \omega' \\ v'_{lin} \end{bmatrix} = \begin{bmatrix} I & 0 \\ -[p]_{\times} & I \end{bmatrix} \begin{bmatrix} \omega \\ v_{lin} \end{bmatrix}
```
...
```math
\begin{bmatrix} \omega'' \\ v''_{lin} \end{bmatrix} = \begin{bmatrix} R & 0 \\ 0 & R \end{bmatrix} \begin{bmatrix} \omega' \\ v'_{lin} \end{bmatrix}
```
...
```math
X = \begin{bmatrix} R & 0 \\ -R[p]_{\times} & R \end{bmatrix} \in \mathbb{R}^{6 \times 6}
```
...
```math
X^{-T} = \begin{bmatrix} R & -[p]_{\times}^T R \\ 0 & R \end{bmatrix} = \begin{bmatrix} R & [p]_{\times} R \\ 0 & R \end{bmatrix}
```
...
링크 $i$ 의 공간 관성 행렬 $\mathcal{I}\_{i} \in \mathbb{R}^{6 \times 6}$ 을 유도한다.

링크의 물리적 성질: 질량 $m$, 무게 중심(CoM) $c \in \mathbb{R}^3$ (링크 좌표계에서), 관성 텐서 $I\_{cm} \in \mathbb{R}^{3 \times 3}$ (CoM 좌표계에서).
...
```math
T = \frac{1}{2} \omega^T I_{cm} \omega + \frac{1}{2} m \|v_{cm}\|^2
```

여기서

```math
v_{cm} = v_{lin} + \omega \times c = v_{lin} - [c]_{\times} \omega
```
...
```math
T = \frac{1}{2} \mathbf{v}^T \mathcal{I} \mathbf{v}
```
...
```math
2T = \omega^T I_{cm} \omega + m(v_{lin} - [c]_{\times} \omega)^T(v_{lin} - [c]_{\times} \omega)
```

**(2.13b)**

```math
= \omega^T I_{cm} \omega + m v_{lin}^T v_{lin} - 2m v_{lin}^T [c]_{\times} \omega + m \omega^T [c]_{\times}^T [c]_{\times} \omega
```
...
```math
2T = \omega^T(I_{cm} + m[c]_{\times} [c]_{\times}^T)\omega + 2m v_{lin}^T [c]_{\times}^T \omega + m v_{lin}^T v_{lin}
```

이를 $\mathbf{v} = [\omega; v\_{lin}]^T$ 에 대한 이차형식으로 쓰면:

**(2.14)**

```math
\mathcal{I} = \begin{bmatrix} I_{cm} + m[c]_{\times} [c]_{\times}^T & m[c]_{\times} \\ m[c]_{\times}^T & m I_3 \end{bmatrix}
```
...
```math
[c]_{\times} [c]_{\times}^T = -[c]_{\times}^2 \geq 0
```
...
```math
\mathcal{I}_{new} = X^{-T} \mathcal{I} X^{-1}
```

여기서 $X$ 는 식 (2.8)의 공간 변환 행렬이다. 이를 전개하면 회전 관성에

```math
m [d]_{\times} [d]_{\times}^T
```

 가 추가되는 표준 평행축 정리와 일치한다.
...
```math
\mathcal{I}_{11} = I_{cm} + m[c]_{\times} [c]_{\times}^T
```

 는 대칭,

```math
\mathcal{I}_{22} = mI_{3}
```

 는 대칭, 그리고

```math
\mathcal{I}_{12} = m[c]_{\times}
```

,

```math
\mathcal{I}_{21} = m[c]_{\times}^T = \mathcal{I}_{12}^T
```

. 따라서 $\mathcal{I}$ 는 대칭이다.
...
```math
[\mathbf{v}_1]_{\times} = \begin{bmatrix} [\omega_1]_{\times} & 0 \\ [v_1]_{\times} & [\omega_1]_{\times} \end{bmatrix} \in \mathbb{R}^{6 \times 6}
```

이는 두 공간 속도의 리 괄호(Lie Bracket)

```math
[\mathbf{v}_{1}, \mathbf{v}_{2}] = [\mathbf{v}_{1}]_{\times} \mathbf{v}_{2}
```
...
```math
[\mathbf{v}]_{\times^*} = -[\mathbf{v}]_{\times}^T
```

**유도**: 파워 $P = \mathbf{f}^T \mathbf{v}\_{2}$ 의 시간 미분에서 힘에 대한 크로스곱이 등장한다. 파워 보존 조건 $\delta P = 0$ 에서:

**(2.18)**

```math
\mathbf{f}^T ([\mathbf{v}_1]_{\times} \mathbf{v}_2) = ([\mathbf{v}_1]_{\times^*} \mathbf{f})^T \mathbf{v}_2
```

이므로

```math
[\mathbf{v}_{1}]_{\times^*} = [\mathbf{v}_{1}]_{\times}^T
```

... 이때 Featherstone(2008)의 부호 규약에서
...
관절 $i$ 의 운동 부분공간 $S\_{i} \in \mathbb{R}^6$ 는 관절이 허용하는 상대 운동 방향을 나타낸다.

**회전 관절 (Revolute Joint)**: 관절 축 $e\_{axis}$ 방향으로만 회전 가능:

**(2.19)**

```math
S_i = \begin{bmatrix} e_{axis} \\ 0 \end{bmatrix} \in \mathbb{R}^6
```
...
```math
X_i(q_i) = X_{tree,i} \cdot X_{rot}(q_i)
```
...
: 관절 각도 $q\_{i}$ 에 의한 회전 변환

관절 회전 행렬:
- Z축 관절:

```math
R_{rot} = R_{z}(q_{i}) = \begin{bmatrix} \cos q_{i} & -\sin q_{i} & 0 \\ \sin q_{i} & \cos q_{i} & 0 \\ 0 & 0 & 1 \end{bmatrix}
```
...
```math
R_{rot} = R_{y}(q_{i}) = \begin{bmatrix} \cos q_{i} & 0 & \sin q_{i} \\ 0 & 1 & 0 \\ -\sin q_{i} & 0 & \cos q_{i} \end{bmatrix}
```
...
```math
R_{rot} = R_{x}(q_{i}) = \begin{bmatrix} 1 & 0 & 0 \\ 0 & \cos q_{i} & -\sin q_{i} \\ 0 & \sin q_{i} & \cos q_{i} \end{bmatrix}
```
...
```math
R_{total} = R_{fixed} \cdot R_{rot}(q_i)
```
...
```math
X_{world,0} = X_{base}(R_{base}(q_{quat}), p_{base})
```
...
```math
X_{world,i} = X_i(q_i) \cdot X_{world,parent(i)}
```

> **구현 주의**: `thor/model/kinematics.py` 에서의 실제 코드는

```math
X_{world,i} = X_{parent,i} \cdot X_{world,parent(i)}
```

 형태로, 변환은 왼쪽에서 오른쪽으로 합성된다. 이는 Featherstone(2008)의 규약으로, 변환이 "from world to body"를 의미한다.
...
```math
p = \begin{bmatrix} -(-p_{\times})_{2,1} \\ -(-p_{\times})_{0,2} \\ -(-p_{\times})_{1,0} \end{bmatrix} = \begin{bmatrix} (p_{\times})_{2,1} \\ (p_{\times})_{0,2} \\ (p_{\times})_{1,0} \end{bmatrix}
```
...
```math
c = \frac{1}{M_{total}} \sum_{i=0}^{N-1} m_i \left( p_i + R_i \cdot c_{body,i} \right)
```
...
```math
M_{total} = \sum_{i} m_{i}
```

 : 전체 질량
- $p\_{i}$ : 몸체 $i$ 의 원점 위치 (세계 좌표계)
-
...
- $c\_{body,i}$ : 몸체 $i$ 의 CoM (링크 좌표계에서)
...
```math
J_{i} \in \mathbb{R}^{6 \times n_{dof}}
```

 은 일반화 속도 $v$ 를 공간 속도 $\mathbf{v}\_{i}$ 로 매핑한다:

**(3.9)**

```math
\mathbf{v}_i = J_i \cdot v
```
...
```math
J_i[:, 0:6] = X_{world,i} \cdot X_{world,0}^{-1}
```

**회전 관절 열 (Revolute Joint Column, 관절 $j$)**:

관절 $j$ 의 운동 부분공간 $S\_{j}$ 를 몸체 $i$ 의 프레임으로 변환 (단, $j$ 는 $i$ 의 조상):

**(3.11)**

```math
J_i[:, 5+j] = X_{world,i} \cdot X_{world,j}^{-1} \cdot S_j
```
...
자코비안 $J\_{i}$ 의 정확성을 유한 차분으로 검증한다:

**(3.12)**

```math
J_i[:, k] \approx \frac{p_i(q + \delta e_k) - p_i(q - \delta e_k)}{2\delta}
```

여기서 $e\_{k}$ 는 $k$ 번째 표준 기저 벡터, $\delta = 10^{-6}$ 이다.
...
)에 대해 검증하면 $O(\delta^2)$ 의 오차가 예상된다.
...
| 연산 | 횟수 | 비고 |
|:---|:---:|:---|
| 관절 변환 $X\_i(q\_i)$ | 34 | 각 관절마다 1회 |
| 공간 변환 합성 $X\_{world}$ | 34 | 부모→자식 전파 |
| 위치 추출 $p\_i$ | N/A | 필요 시만 계산 |
| 자코비안 $J\_i$ | $O(d\_i)$ | $d\_i$ = 체인 깊이 |
...
```math
J_{c}^T \in \mathbb{R}^{40 \times 3n_{c}}
```
...
```math
M = \begin{bmatrix} M_{bb} & M_{bj} \\ M_{jb} & M_{jj} \end{bmatrix}
```
...
```math
M_{bj} = M_{jb}^T \in \mathbb{R}^{6 \times 34}
```

: 기저-관절 결합 항
- $M\_{jj} \in \mathbb{R}^{34 \times 34}$: 관절 공간 관성
...
```math
S^T = \begin{bmatrix} 0_{6 \times 34} \\ I_{34 \times 34} \end{bmatrix}
```
...
**물리적 정당성**: 모든 몸체에 중력이 작용하는 것은, 기저에 위쪽 방향 가상 가속도 $a\_{grav}$ 를 더하는 것과 등가다.
...
```math
m_i \ddot{p}_i = f_{ext,i} + m_i g \hat{z}
```

관성 프레임을 기준으로 취하면, 기저에 가속도 $-g \hat{z}$ 를 적용하는 것은 모든 몸체에 중력 $m\_{i} g \hat{z}$ 를 작용시키는 것과 등가다 (등가 원리, Equivalence Principle).
...
```math
a_{grav} = \begin{bmatrix} 0 \\ 0 \\ 0 \\ 0 \\ 0 \\ +g \end{bmatrix} = [0; 0; 0; 0; 0; +9.81]^T
```
...
```
 ...
 에 대해, 운동 에너지 $T = \frac{1}{2} v^T M v$ 로부터 $M = M^T$. CRBA에서도 오프대각 원소가 대칭으로 채워짐을 확인할 수 있다 (5.2절 참조).
...
```math
\frac{d}{dt}\frac{\partial L}{\partial v} - \frac{\partial L}{\partial q} = Q
```

여기서

```math
Q = S^T \tau + J_{c}^T f_{c}
```
...
```math
M\dot{v} + \dot{M} v - \frac{1}{2} v^T \frac{\partial M}{\partial q} v + \frac{\partial V}{\partial q} = Q
```
...
조건수 1330은 double precision ($\epsilon\_{machine} \approx 10^{-16}$)에서 약 $10^{-13}$ 의 상대 오차를 의미한다. 이는 CRBA-RNEA 교차 검증에서 관측된 $1.14 \times 10^{-13}$ N·m 오차와 일치한다.
...
```math
\mathbf{v}_i = X_{up,i} \mathbf{v}_{parent(i)} + S_i \dot{q}_i
```
...
```math
\mathbf{a}_i = X_{up,i} \mathbf{a}_{parent(i)} + S_i \ddot{q}_i + \underbrace{[\mathbf{v}_i]_{\times} S_i \dot{q}_i}_{\text{속도곱 가속도}}
```

**속도곱 가속도 항 (Velocity-Product Acceleration)**:

```math
[\mathbf{v}_{i}]_{\times} S_{i} \dot{q}_{i}
```
...
유도: 관절 $i$ 의 공간 속도 $\mathbf{v}\_{i}$ 는 부모의 속도 전파와 관절 기여의 합이다. 시간 미분하면:

**(5.5)**

```math
\dot{\mathbf{v}}_i = \dot{X}_{up,i} \mathbf{v}_{parent} + X_{up,i} \dot{\mathbf{v}}_{parent} + \dot{S}_i \dot{q}_i + S_i \ddot{q}_i
```



```math
\dot{X}_{up,i} \mathbf{v}_{parent} = [\mathbf{v}_{i}]_{\times} X_{up,i} \mathbf{v}_{parent}
```

 이고 $\dot{S}\_{i} = 0$ (운동 부분공간은 관절 좌표계에서 상수)이므로:

**(5.6)**

```math
\mathbf{a}_i = X_{up,i} \mathbf{a}_{parent} + S_i \ddot{q}_i + [\mathbf{v}_i]_{\times} (S_i \dot{q}_i)
```
...
```math
\dot{R}_{i} = [\omega_{i}]_{\times} R_{i}
```
...
```math
\dot{X}_{up,i} = \begin{bmatrix} [\omega_i]_{\times} R_i & 0 \\ \cdots & [\omega_i]_{\times} R_i \end{bmatrix} = [\mathbf{v}_i^{joint}]_{\times} X_{up,i}
```

여기서

```math
\mathbf{v}_{i}^{joint} = S_{i} \dot{q}_{i}
```

 는 관절이 기여하는 공간 속도다. 따라서:

**(5.5b)**

```math
\dot{X}_{up,i} \mathbf{v}_{parent} = [\mathbf{v}_i^{joint}]_{\times} (X_{up,i} \mathbf{v}_{parent}) \approx [\mathbf{v}_i]_{\times} (X_{up,i} \mathbf{v}_{parent}) \quad \checkmark
```
...
```math
\mathbf{f}_i = \mathcal{I}_i \mathbf{a}_i + [\mathbf{v}_i]_{\times^*} (\mathcal{I}_i \mathbf{v}_i)
```
...
```math
[\mathbf{v}_{i}]_{\times^*} (\mathcal{I}_{i} \mathbf{v}_{i}) = \mathbf{v}_{i} \times^* (\mathcal{I}_{i} \mathbf{v}_{i})
```

: 자이로스코픽 힘 (Gyroscopic Force) — 회전 운동에 의한 원심 및 코리올리 효과
...
```math
\mathbf{f}_{parent(i)} \mathrel{+}= X_{up,i}^T \mathbf{f}_i
```
...
```math
\text{RNEA}(q, v, \dot{v}) = M(q) \dot{v} + h(q, v)
```
...
```math
M(q) \in \mathbb{R}^{n_{dof} \times n_{dof}}
```

 를 $O(Nd)$ 복잡도로 계산한다. 여기서 $d$ 는 트리 깊이(THOR에서 약 7).

#### 5.2.1 Pass 1: 합성 관성 누적 (Composite Inertia Accumulation)

잎에서 루트 방향으로 합성 관성 $\mathcal{I}\_{c}^{(i)}$ 를 계산한다.

초기값:

```math
\mathcal{I}_{c}^{(i)} = \mathcal{I}_{i}
```
...
```math
\mathcal{I}_c^{(parent(i))} \mathrel{+}= X_{up,i}^T \mathcal{I}_c^{(i)} X_{up,i}
```
...
```math
M[dof_i, dof_i] = S_i^T \mathcal{I}_c^{(i)} S_i
```
...
```math
F_{up} \leftarrow X_{up,j}^T F_{up}
```

**(5.14b)**

```math
M[dof_i, dof_j] = M[dof_j, dof_i] = S_j^T F_{up}
```
...
```math
\|M(q) \dot{v} + h(q, v) - \text{RNEA}(q, v, \dot{v})\|_\infty < \epsilon_{machine}
```

### 5.3 ABA — 전진 동역학 (Articulated Body Algorithm)

ABA는 주어진 $(q, v, \tau, f\_{ext})$ 에서 가속도 $\dot{v}$ 를 $O(N)$ 으로 계산한다:

**(5.16)**

```math
\dot{v} = \text{ABA}(q, v, \tau, f_{ext})
```
...
```math
c_i = [\mathbf{v}_i]_{\times} S_i \dot{q}_i
```

초기 관절체 관성(Articulated Body Inertia):
...
```math
pA_i = [\mathbf{v}_i]_{\times^*} (\mathcal{I}_i \mathbf{v}_i)
```
...
```math
IA_i^a = IA_i - \frac{U_i U_i^T}{D_i}
```

**(5.20b)**

```math
pA_i^a = pA_i + IA_i^a c_i + U_i \frac{u_i}{D_i}
```

**(5.20c)**

```math
IA_{parent(i)} \mathrel{+}= X_{up,i}^T IA_i^a X_{up,i}
```

**(5.20d)**

```math
pA_{parent(i)} \mathrel{+}= X_{up,i}^T pA_i^a
```
...
```math
\mathbf{a}_0 = IA_0^{-1} (\tau_{0:6} - pA_0) + a_{grav}
```
...
```math
a_{parent} = X_{up,i} \mathbf{a}_{parent(i)} + c_i
```

**(5.22b)**

```math
\ddot{q}_i = \frac{u_i - U_i^T a_{parent}}{D_i}
```

**(5.22c)**

```math
\mathbf{a}_i = a_{parent} + S_i \ddot{q}_i
```
...
```math
M \cdot \dot{v} + h = \text{RNEA}(q, v, \dot{v}) \quad \forall \dot{v}
```
...
```math
\epsilon = \| \text{CRBA}(q) \cdot \dot{v}_{test} + \text{RNEA}(q, v, 0) - \text{RNEA}(q, v, \dot{v}_{test}) \|_\infty
```
...
CRBA의 $M[:6,:6] = \mathcal{I}\_{c}^{(0)}$ 는 전체 시스템을 기저에 고정된 단일 강체로 볼 때의 관성과 같다.

**(5.26)**

```math
\mathcal{I}_c^{(0)} = \sum_{i=0}^{N-1} X_{0,i}^{-T} \mathcal{I}_i X_{0,i}^{-1}
```

여기서 $X\_{0,i}$ 는 기저 프레임에서 몸체 $i$ 로의 변환이다. 이는 루트에서 누적된 합성 관성이며, Newton-Euler의 전체 모멘텀 방정식을 그대로 반영한다.
...
중심 운동량(Centroidal Momentum) $h\_{G} \in \mathbb{R}^6$ 은 전체 시스템의 CoM 프레임에서 본 각운동량과 선 운동량이다:

**(6.1)**

```math
h_G = \begin{bmatrix} k_G \\ l_G \end{bmatrix} = A_G(q) \cdot v
```

여기서:
- $k\_{G} \in \mathbb{R}^3$: CoM에서의 각운동량 (Angular Momentum)
-
...
```math
A_{G}(q) \in \mathbb{R}^{6 \times n_{dof}}
```
...
```math
A_G = \sum_{i=0}^{N-1} X_{G,i}^{-T} \mathcal{I}_i J_i
```

여기서 $X\_{G,i}$ 는 몸체 $i$ 에서 CoM 프레임까지의 공간 변환이다.

구체적 계산: 몸체 $i$ 의 CoM 위치 $c\_{i}$ 와 전체 CoM $c$ 의 차이
...
```math
A_G[:3, :] \mathrel{+}= (R_i I_{cm,i} R_i^T) J_{\omega,i} + m_i [r_i]_{\times} J_{v,i}
```
...
```math
J_{v,i} \in \mathbb{R}^{3 \times n_{dof}}
```

: 몸체 $i$ 자코비안의 선속도 부분 (하위 3행)
-

```math
J_{\omega,i} \in \mathbb{R}^{3 \times n_{dof}}
```

: 몸체 $i$ 자코비안의 각속도 부분 (상위 3행)
-
...
```math
\dot{h}_G = \sum_i f_{ext,i} + \begin{bmatrix} 0 \\ M_{total} g \hat{z} \end{bmatrix}
```
...
```math
\dot{l}_G = M_{total} \ddot{c} = \sum_i f_i + M_{total} g \hat{z}
```

**(6.5b)**

```math
\dot{k}_G = \sum_i (p_i - c) \times f_i + \sum_i \tau_{ext,i}
```

### 6.4 선형 역진자 모델 (LIPM, Linear Inverted Pendulum Model)

보행 제어의 단순화를 위해 CoM 높이를 $z\_{0}$ 로 고정하는 3D LIPM을 사용한다.
...
```math
\ddot{c}_x = \frac{g}{z_0}(c_x - p_{zmp,x})
```

**(6.6b)**

```math
\ddot{c}_y = \frac{g}{z_0}(c_y - p_{zmp,y})
```

여기서 $p\_{zmp}$ 는 영모멘트 점(Zero Moment Point, ZMP)이다.

**유도**: 발 반력이 ZMP에 집중되고 CoM 높이가 일정할 때, 각운동량 $\dot{k}\_{G} = 0$ 조건에서 식 (6.4)를 수평면으로 투영하면 식 (6.6)을 얻는다.
...
```math
\underbrace{\begin{bmatrix} \dot{c} \\ \ddot{c} \end{bmatrix}}_{\dot{x}} = \underbrace{\begin{bmatrix} 0 & 1 \\ g/z_0 & 0 \end{bmatrix}}_{A} \underbrace{\begin{bmatrix} c \\ \dot{c} \end{bmatrix}}_{x} + \underbrace{\begin{bmatrix} 0 \\ 1 \end{bmatrix}}_{B} u
```
...
```math
f_n = k_n \max(0, -\phi) + d_n \max(0, -\dot{\phi})
```

여기서:
- $k\_{n} = 3000 \;\text{[N/m]}$: 법선 강성 (Normal Stiffness)
- $d\_{n} = 300 \;\text{[N·s/m]}$: 법선 감쇠 (Normal Damping)
- $\dot{\phi}$: 접촉점의 법선 방향 속도

**파라미터 정당성**:
- THOR 질량 $M = 65$ kg, 중력 $g = 9.81$ m/s²
- 정적 하중: $F\_{g} = 65 \times 9.81 = 637.65$ N
- 8개 접촉점 (발 각 4개) 분산 시, 점당 $\approx 80$ N
- 정적 침투 깊이:

```math
\phi_{static} = F_{g} / (8 k_{n}) = 637.65 / 24000 \approx 0.027
```
...
```math
d_{crit} = 2\sqrt{m_{foot} k_{n}} = 2\sqrt{3 \times 3000} \approx 190
```

 N·s/m → $d\_{n} = 300$ 은 과감쇠(Overdamped) 영역
...
```math
\mathbf{f}_t = -\mu f_n \tanh\left(\frac{v_t}{v_s}\right) \frac{\mathbf{v}_t}{|\mathbf{v}_t|}
```
...
: 접선 속도 크기
- $v\_{s} = 0.01$ m/s: 정지 속도 임계값(Stiction Velocity Threshold)
...
```math
\tanh(v_{t}/v_{s}) \approx v_{t}/v_{s}
```
...
```math
v_{corner} = v_{foot} + \omega_{foot} \times r
```
...
```math
\phi_n \geq 0, \quad \lambda_n \geq 0, \quad \phi_n \lambda_n = 0
```

이를 간결하게:

```math
0 \leq \lambda_{n} \perp \phi_{n} \geq 0
```
...
```math
w = M_{LCP} \lambda + q_{LCP}, \quad 0 \leq \lambda \perp w \geq 0
```
...
```math
M_{LCP} = J_n M^{-1} J_n^T
```



```math
J_{n} \in \mathbb{R}^{n_{c} \times n_{dof}}
```
...
```math
M v_{k+1} = M v_k + h_{step}(S^T \tau - h) + J_n^T \lambda_n
```
...
```math
v_{k+1} = v_k + M^{-1}(h_{step}(S^T \tau - h) + J_n^T \lambda_n)
```

법선 방향 속도 제약 (

```math
J_{n} v_{k+1} \geq -\phi_{n}/h_{step}
```
...
```math
J_n v_{k+1} = J_n v_k + J_n M^{-1}(h_{step}(S^T\tau - h)) + J_n M^{-1} J_n^T \lambda_n
```
...
```math
\underbrace{J_n M^{-1} J_n^T}_{M_{LCP}} \lambda_n + \underbrace{J_n v_k + J_n M^{-1} h_{step}(S^T\tau - h) + \phi_n/h_{step}}_{q_{LCP}} = w
```

상보성 조건: $0 \leq \lambda\_{n} \perp w \geq 0$ $\quad \blacksquare$
...
```math
\phi_{FB}(a, b; \varepsilon) = a + b - \sqrt{a^2 + b^2 + 2\varepsilon^2}
```

**성질**:
1. $\phi\_{FB}(a, b; 0) = 0 \iff a \geq 0, b \geq 0, ab = 0$ (상보성 조건과 등가)
2. $\varepsilon > 0$ 이면 $\phi\_{FB}$ 는 도처에서 미분 가능 (Differentiable everywhere)
...
```math
\frac{\partial \phi_{FB}}{\partial a} = 1 - \frac{a}{\sqrt{a^2 + b^2 + 2\varepsilon^2}}
```

**(7.9b)**

```math
\frac{\partial \phi_{FB}}{\partial b} = 1 - \frac{b}{\sqrt{a^2 + b^2 + 2\varepsilon^2}}
```

LCP $w = M\lambda + q$ 에 식 (7.8)를 적용하면,
...
```math
b_{i} = w_{i} = (M\lambda + q)_{i}
```

 이고 전체 잔차 벡터 $F(\lambda)$ 의 야코비안:

**(7.10)**

```math
J_{ij} = \frac{\partial \phi_{FB,i}}{\partial \lambda_j} = \frac{\partial \phi_{FB}}{\partial b_i} M_{ij} + \delta_{ij} \frac{\partial \phi_{FB}}{\partial a_i}
```
...
3. 역추적 선 탐색: $\alpha$ 를 $\lVert F(\lambda + \alpha \Delta\lambda)\rVert < \lVert F(\lambda)\rVert$ 가 될 때까지 반분
...
```math
\lambda_i w_i = \kappa \quad (\kappa > 0)
```
...
```math
M(v_{k+1} - v_k) = h_{step}(S^T \tau - h) + J_c^T \lambda
```
...
```math
\begin{bmatrix} M_{bb} & M_{bj} \\ M_{jb} & M_{jj} \end{bmatrix} \begin{bmatrix} \dot{v}_b \\ \dot{v}_j \end{bmatrix} + \begin{bmatrix} h_b \\ h_j \end{bmatrix} = \begin{bmatrix} f_b \\ \tau_j \end{bmatrix}
```

여기서 $f\_{b}$ 는 기저 반력이다.

**제약**: 이중 지지에서 $\dot{v}\_{b} = 0$ 으로 설정한다. 두 번째 행 방정식:

**(7.16a)**

```math
M_{jb} \cdot 0 + M_{jj} \dot{v}_j + h_j = \tau_j
```
...
```math
M_{bb} \cdot 0 + M_{bj} \dot{v}_j + h_b = f_b
```

**(7.18)**

```math
f_b = M_{bj} \dot{v}_j + h_b
```
...
**Cholesky 분해의 사용**: $M\_{jj}$ 는 대칭 양정치이므로 Cholesky 분해를 사용하면 LU 분해 대비 절반의 연산으로 선형 시스템을 풀 수 있다.

**(7.19)**

```math
M_{jj} = L L^T, \quad L \Delta v_j = \tau_j - h_j
```
...
```math
J = \sum_{k=0}^{N-1} \left( \|q_k - q_{ref,k}\|_{Q_q}^2 + \|v_k - v_{ref,k}\|_{Q_v}^2 + \|\tau_k\|_R^2 \right)
```
...
```math
Q_{q} = \text{diag}(q_{pos} \cdot \mathbf{1})
```
...
```math
Q_{v} = \text{diag}(q_{vel} \cdot \mathbf{1})
```
...
```math
\tau = g(q) + K_p (q_{ref} - q) + K_v (v_{ref} - v)
```
...
```math
M_{jj} \ddot{q} = \tau - h_j = M_{jj} \ddot{q}_{des} + h_j - h_j = M_{jj} \ddot{q}_{des}
```
...
```math
\ddot{q}_{des} = \ddot{q}_{ref} - K_p (q - q_{ref}) - K_d (\dot{q} - \dot{q}_{ref})
```

추적 오차 $e = q - q\_{ref}$ 의 동역학:
...
원하는 자연진동수 $\omega\_{n}$, 감쇠비 $\zeta$ 에 대해:
...
```math
\omega_n = \sqrt{600} \approx 24.5 \;\text{rad/s}, \quad \zeta = \frac{60}{2 \times 24.5} \approx 1.22 \;(\text{과감쇠})
```
...
**보행 위상 파라미터**:
- 이중 지지(Double Support) 지속 시간: $T\_{DS} = 0.25$ s
- 스윙(Swing) 지속 시간: $T\_{swing} = 0.55$ s
- 보행 사이클:

```math
T_{cycle} = T_{DS} + T_{swing} = 0.80
```
...
```math
\theta_{ankle}(s) = \theta_{swing} \cdot \sin(\pi s)
```

여기서 $\theta\_{swing} = 5°$ (배측굴곡, Dorsiflexion).
...
```math
\theta_{hip}^{stance}(s) = \theta_{flex}(1-s) + \theta_{ext} s
```
...
```math
\theta_{knee}^{stance}(s) = \theta_{stance} + 10° \cdot \sin\left(\frac{\pi s}{2}\right)
```
...
```math
\theta_{ankle}^{stance}(s) = 5° \cdot \sin(\pi s) + \theta_{push} \cdot s^2
```

여기서 $\theta\_{push} = -10°$ (추진, Push-off).
...
```math
\theta = (1 - \beta) \theta_{end} + \beta \theta_{start}'
```
...
```math
u = -K \begin{bmatrix} c - c_{des} \\ \dot{c} - \dot{c}_{des} \end{bmatrix}
```
...
```math
\min_{\tau, f} \|A_G(q) v - h_{G,ref}\|^2 + \lambda \|\tau\|^2
```
...
```
 ...
 는 마찰 원뿔(Friction Cone) 내에 있어야 한다:
...
각도 $\theta\_{k} = 2\pi k / 8$, $k = 0, \ldots, 7$ 에 대해:

**(8.22)**

```math
\cos\theta_k f_x + \sin\theta_k f_y \leq \mu f_n
```
...
```math
\tau_{PD} = K_p(q_{ref} - q) + K_d(\dot{q}_{ref} - \dot{q})
```
...
계산 토크 제어에서는 $h\_{j}$ 에 중력이 포함되어 자동으로 보상된다.
...
)와 달리, 반암시적 오일러는 설정 업데이트에 **새 속도** $v\_{k+1}$ 을 사용한다.
...
```math
\frac{d\mathbf{q}}{dt} = \frac{1}{2} \begin{bmatrix} 0 \\ \omega \end{bmatrix} \otimes \mathbf{q}
```
...
```
 ...
 에서 출발하여 쿼터니언 표현으로 변환하면 식 (9.3)을 얻는다.
...
```math
\frac{d}{dt}\begin{bmatrix} w \\ x \\ y \\ z \end{bmatrix} = \frac{1}{2} \begin{bmatrix} -\omega_x x - \omega_y y - \omega_z z \\ \omega_x w + \omega_z y - \omega_y z \\ \omega_y w - \omega_z x + \omega_x z \\ \omega_z w + \omega_x y - \omega_y x \end{bmatrix}
```
...
```math
\mathbf{q}_{k+1} = \mathbf{q}_k + h \cdot \frac{d\mathbf{q}_k}{dt}
```
...
```math
q_{k+1} = q_{k} + h v_{k+1} = q_{k} + h v_{k} + h^2 \dot{v}_{k} + O(h^3)
```
...
```math
q(t_{k+1}) = q_{k} + h v_{k} + \frac{h^2}{2}\dot{v}_{k} + O(h^3)
```
...
```math
q_{k+1} - q(t_{k+1}) = \frac{h^2}{2}\dot{v}_{k} + O(h^3)
```
...
```math
|H(q_{k}, v_{k}) - H(q_0, v_0)| = O(h) \quad \text{(유계, 선형 증가 없음)}
```
...
```math
|H(q_{k}, v_{k}) - H(q_0, v_0)| \sim O(e^{h \Lambda_{max} t})
```
...
공간 속도 규약 확인: `v[0:3]` = 각속도 $\omega$, `v[3:6]` = 선속도 $v\_{lin}$ (omega-first, 식 2.1)
...
| $J\_c$ | $\mathbb{R}^{3n\_c \times 40}$ | 접촉 자코비안 |
| $f\_c$ | $\mathbb{R}^{3n\_c}$ | 접촉력 벡터 |
...
| $S\_i$ | $\mathbb{R}^6$ | 관절 $i$ 의 운동 부분공간 |
| $X\_{up,i}$ | $\mathbb{R}^{6 \times 6}$ | 몸체 $i$ 에서 부모로의 공간 변환 |
| $\mathcal{I}\_c^{(i)}$ | $\mathbb{R}^{6 \times 6}$ | 몸체 $i$ 의 합성 관성 (CRBA) |
| $IA\_i$ | $\mathbb{R}^{6 \times 6}$ | 몸체 $i$ 의 관절체 관성 (ABA) |
| $pA\_i$ | $\mathbb{R}^6$ | 몸체 $i$ 의 관절체 바이어스 힘 (ABA) |
| $A\_G$ | $\mathbb{R}^{6 \times 40}$ | 중심 운동량 행렬 (CMM) |
| $h\_G$ | $\mathbb{R}^6$ | 중심 운동량 (각운동량 + 선운동량) |
| $c$ | $\mathbb{R}^3$ | 전체 시스템 질량 중심 위치 |
| $R$ | $\mathbb{R}^{3 \times 3}$ | 회전 행렬 ($\in SO(3)$) |
| $[p]\_\times$ | $\mathbb{R}^{3 \times 3}$ | 벡터 $p$ 의 반대칭 행렬 (Skew-Symmetric) |
| $\phi\_{FB}$ | $\mathbb{R}$ | Fischer-Burmeister NCP 함수 |
| $\lambda$ | $\mathbb{R}^{n\_c}$ | LCP 해 (접촉 법선력) |
| $h\_{step}$ | $\mathbb{R}$ | 시간 스텝 크기 |
| $N$ | 정수 | 몸체 수 ($=35$, THOR) |
| $n\_{dof}$ | 정수 | DOF 수 ($=40$, THOR) |
...
> **부록 B–G**: [THEORY_APPENDIX.md](THEORY_APPENDIX.md) — 스큐 대칭 행렬, 회전 행렬 미분, 좌표 변환, 수치 구현, 안정성 분석, 물리 파라미터
