...
**검증:** THOR 모델에서 $M(\mathbf{q})$는 40×40, 대칭($\|M - M^T\| < 10^{-8}$), 양정치이며, 병진 블록 $M[3{:}6,3{:}6] = 67.2 \cdot I_3$ (총 질량)를 만족한다.
...
| M 병진 블록 | $M[3{:}6,3{:}6] = 67.2 \cdot I_3$ (총 질량) |
...
|:---|:---|:---|:---|
| 부유 기저 적분 발산 | $M_{bj}$ 결합이 기저 각가속도 (~47 rad/s²)를 관절에 전파 | **Schur 보완 소거**: $M_{jj}\ddot{\mathbf{q}}_j = \boldsymbol{\tau}_j - \mathbf{h}_j$ (34×34만) 풀기 | 8.2 |
| 보행 중 관절 동결 | Coriolis $C\dot{\mathbf{q}}$가 편향 구성에서 PD와 균형 (가짜 평형) | **계산 토크 제어**: $\boldsymbol{\tau} = M\ddot{\mathbf{q}}_{\mathrm{des}} + \mathbf{h}$로 $\ddot{\mathbf{q}} = \ddot{\mathbf{q}}_{\mathrm{des}}$ 보장 | 8.3 |
| 보행 중 로봇 회전 | 속도 규약 교환: 위치 업데이트에 $\omega$, 쿼터니언 업데이트에 $\mathbf{v}_{\mathrm{lin}}$ 사용 | 수정: position += $h \cdot v[3{:}6]$, quat += $h \cdot v[0{:}3]$ (Featherstone) | 6.2 |
| LCP 부호 오류 → 영 접촉력 | Schur 보완: $\lambda = A^{-1}b$ (틀림), $\lambda = A^{-1}(-b)$이어야 함 | $b = -J_n M^{-1}(\boldsymbol{\tau} - \mathbf{h})$의 부호 수정 | 6.4 |
